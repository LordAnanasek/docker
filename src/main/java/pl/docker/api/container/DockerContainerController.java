...
public class DockerContainerController implements IDockerContainerController {
    ...
    @Override
    public void createAndRunContainer(@RequestBody DockerContainerCreateDto dockerContainerCreateDto){
        dockerManagement.createAndRunContainer(dockerContainerCreateDto);
    }

    @CrossOrigin(exposedHeaders = ACCESS_CONTROL_ALLOW_ORIGIN)
    @RequestMapping(value = "api/container/{id}", method = RequestMethod.OPTIONS)
    @Override
    public void runContainer(@PathVariable String id) {
        dockerManagement.runContainer(id);
    }
}
